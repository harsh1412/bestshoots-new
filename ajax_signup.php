...
if ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest') {
    ...
    if (!preg_match("/^[а-яА-ЯёЁіІa-zA-Z0-9-_. ]+$/u", $name)) {
        $error["name"] = "name";
        $error["error"] = "Select the name of a length of 3 to 30 characters. You can use letters, numbers, point(.) underscore (_), hyphens (-) and a space";
        exit(json_encode($error));
    }
    ...
    $sql = "SELECT `col_company_name` FROM `tbl_users` WHERE `col_company_name`='" . $name . "' ";
    $query = mysqli_query($link, $sql);

    $name_num = mysqli_num_rows($query);
    if ($name_num == '1') {
        $error["name"] = "name";
        $error["error"] = "This name is already in use, enter another";
        exit(json_encode($error));
    }
    if ($email == "") {
        $error["name"] = "email";
    ...
    $sql2 = "SELECT `col_email` FROM `tbl_users` WHERE `col_email`='" . $email . "' ";
    $query2 = mysqli_query($link, $sql2);

    $email_num = mysqli_num_rows($query2);
    if ($email_num == '1') {
        $error["name"] = "email";
    ...
    $query3 = mysqli_query($link, $insert);
    ...
    if ($query3) {
        ...
        $error["name"] = "signup";
        $error["redirect"] = "/company_profile.php?id=" . $id;
        exit(json_encode($error));
    }
} //кінець ajax